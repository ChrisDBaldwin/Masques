...
- **ClickHouse schema** on your-clickhouse-host (`sql/001-006`): identity tables, ledger tables (TigerBeetle mirrors), settlements, API request metering, reputation scoring. See `sql/README.md` for the full architecture diagram.
- **OTEL collector** running locally, shipping metrics and logs to your-clickhouse-host's ClickHouse (`otel` database). Config in `services/collector/`. Uses `.env` for connection details (gitignored).
...
4. **Integration point** — Where does TigerBeetle run? On your-clickhouse-host alongside ClickHouse? Locally? The masque plugin needs to communicate with it on don/doff events. Direct client? Via an API service?
