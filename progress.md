...
### Iteration 8 — Roll rubrics out to the full roster + bump versions (user request)
Extends D7 beyond the PRD's "one persona" scope at the user's request: every masque now carries a `rubric`, and versions iterate 0.1.0 → 0.2.0.
- **Cull check:** all 35 personas were already structurally spec-clean — top-level keys are only `name/version/attributes/context/lens/spinnerVerbs` (+rubric). Nothing to remove. (`attributes` is `additionalProperties:true`, so no nested culling either.)
- **Authoring:** 5 parallel subagents each took a disjoint 7-file batch; each rubric derived from that masque's own lens/context/tagline ("measurable shadow of the lens"), placed before `spinnerVerbs`, codesmith left as-is. Witness got a judge-specific rubric (reads the *target's* rubric, evidence-grounded, stays in its lane).
- **Versions:** 0.1.0 → 0.2.0 in both the header comment and `version:` field, all 35.
- **Manifest:** `personas/manifest.yaml` regenerated from source (35 entries, all 0.2.0; also fixed a pre-existing Codesmith/Collie mis-ordering).
- **Verification (central, real):**
  - jsonschema (Draft 2020-12): all 35 valid; every file has a non-empty `rubric`, `version: 0.2.0`, and exactly the 7 allowed top-level keys — 0 failures.
  - Content integrity: `git diff` removed-lines across all personas are ONLY the old version lines (header + `version:`) — **no lens/context/spinnerVerbs content altered**. 34 `rubric:` keys added (codesmith's existing one = 35 total).
  - Manifest parses; versions = {0.2.0}; diff vs HEAD is only version + generated_at (+ the benign reorder).
- **Not touched (flagged):** plugin version (`plugin.json`/`marketplace.json`) — these are the *plugin* version, not masque versions; bump separately if you consider the rubric rollout a plugin release. Eval configs not touched (they assert lens content, not version strings).

### Iteration 9 — Broaden Firekeeper to a general DBA + architect (user request)
- **`personas/firekeeper.masque.yaml` v0.2.0 → v0.3.0:** rescoped from a ClickHouse-only, OpenGander-specific masque to a general database administrator + architect. Now fluent across relational/OLTP (PostgreSQL first; MySQL/MariaDB) and columnar/OLAP (ClickHouse + the warehouse family), with explicit guidance on choosing the right store (OLTP vs OLAP), Postgres craft (MVCC, index types, EXPLAIN, partitioning, VACUUM, extensions), ClickHouse/columnar craft (MergeTree, materialized views, sort keys, sharding), and operations (migrations/rollback, tested backups + PITR, replication/failover, observability, capacity/cost). Kept the hearth metaphor, the safety posture (writes→files, never drop/truncate, least privilege, data-isolation boundaries), the spinnerVerbs, and the domain/tagline. Removed all OpenGander/Chris/AWS-specific scoping. Rubric rewritten to the general definition.
- **Manifest:** Firekeeper bumped to 0.3.0 (domain/tagline unchanged).
- **Eval kept in sync (CLAUDE.md invariant):** `evals/firekeeper/prompt.txt` regenerated from the new persona (lens+context+attributes, verbatim; rubric stays out — matches what /don injects); `promptfooconfig.yaml` labels → v0.3.0; comment generalized (tenant→data-isolation). The existing behavioral tests (write-refusal, DROP/TRUNCATE caution, migrations, cost, collaboration, isolation) all remain valid — those boundaries are preserved in the broadened lens.
- **Verification:** jsonschema VALID, v0.3.0, only the 7 allowed keys; content scan: mentions postgres+clickhouse, no opengander; manifest reflects 0.3.0; eval prompt embeds the new lens+context verbatim.
